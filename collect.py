...
def allow_item(entry, feed_name):
    ...
    if feed_name in TRUSTED_SOURCES:
        if any(tok in text for tok in TEAM_TOKENS) or "notre" in text or "irish" in text: return True
...
def uniq_key(link, title):
    return hashlib.md5(((link or "")+"|"+(title or "")).encode("utf-8")).hexdigest()

def parse_time(entry):
    try:
        if getattr(entry, "published_parsed", None): return int(time.mktime(entry.published_parsed))
        if getattr(entry, "updated_parsed", None): return int(time.mktime(entry.updated_parsed))
    except: pass
...
def collect():
    ...
    for f in FEEDS:
        ...
        try:
            ...
            for e in d.entries:
                ...
                if not allow_item(e, name): continue
    ...
    payload = {
        ...
        "count": len(items),
        "items": items,
        "notes": {"errors": errors[:10]},
    }
    with open("items.json","w",encoding="utf-8") as f:
        json.dump(payload, f, ensure_ascii=False, indent=2)
    print(f"[WRITE] items.json — {len(items)} items @ {datetime.now(timezone.utc).isoformat()}")
